...
#' @param i node (default=NULL)
#' @param ... Other arguments
#'
#' @return Subgroup rules/definitions. Based on partykit:::.list.rules.party

## Extract Subgroup Rules (Party Object) ##
list_rules = function (x, i = NULL, ...)
{
  if (is.null(i))
    i <- nodeids(x, terminal = TRUE)
  if (length(i) > 1) {
    ret <- sapply(i, list_rules, x = x)
