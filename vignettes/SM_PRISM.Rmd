...
summ.table = data.frame( `Step` = c("estimand(s)", "filter", "ple", "submod", "param"),
                        ...
                        `survival` = c("HR(A=1 vs A=0)",
                                       "Elastic Net<br>(filter_glmnet)", 
                                       "Random Forest (ranger)<br>(ple_ranger)",
...
where $n(k \cap k_r)$ is the number of subjects in the $k$ who are also in the bootstrap subgroup $k_r$. Bootstrap bias of the observed subgroup estimates ($\hat{\theta}_{k}$) can also be estimated. For each resample and corresponding PRISM fit, predict the subgroup assignment using the observed data ($(Y, A, X)$) and again using the observed data, estimate the corresponding subgroup estimates, $\hat{\theta}_{k_r,obs}$. For resample $r$, the bootstrap estimates for the original identified subgroups ($k=0,...,K$) are calculated as:
$$ \widehat{bias}_{rk} =  \frac{\sum_{k_r} n(k \cap k_r) \hat{(\theta_{k_r}}-\hat{\theta}_{k_r,obs})}{\sum_{k_r} n(k \cap k_r)}  $$
INSERT TEXT and then move to FUNCTION for parameter / CI comparisons. The bootstrap smoothed estimate and standard error can then be calculated as:
