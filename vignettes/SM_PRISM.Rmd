...
Given a candidate set of subgroups, a simple approach is to fit linear regression models within each subgroup to obtain treatment effect estimates (A=1 vs A=0). See below.
...
CIs can then be formed using Z-intervals. For example, a two-sided 95\% CI, $CI_{\alpha}(\hat{\theta}_{k}) = \left[\hat{\theta}_{k} \pm  1.96*SE(\hat{\theta}_k) \right]$
