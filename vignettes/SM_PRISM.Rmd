...
Cross-validation resampling (resample="CV") also follows the same general procedure as bootstrap resampling. Given observed data $(Y, A, X)$, fit $PRISM(Y,A,X)$. Based on the identified $k=1,..,K$ subgroups, output subgroup assignment for each patient. Next, split the data into $R$ folds (ex: 5). For fold $r$ with sample size $n_r$, fit PRISM on $(Y[-r],A[-r], X[-r])$ and predict the patient-level estimates and subgroup assignments for patients in fold $r$. The data in fold $r$ is then used to obtain parameter estimates for each subgroup, $\hat{\theta}_{k_r}$. For parameter estimation models that involve nuisance parameters (i.e. average of individual treatment effects (param_ple) or double robust estimators (param_dr)), this involves For fold $r$, estimates and SEs the original subgroups ($k=1,...,K$) are then obtained using the same formula as with bootstrap resampling, again, denoted as ($\hat{\theta_{rk}}$, $SE(\hat{\theta_{rk}})$). This is repeated for each fold and "CV" estimates and SEs are calculated for each identified subgroup. Let $w_r = n_r / sum(n_r)$, then:

$$ \hat{\theta}_{k,CV} = \sum w_r * \hat{\theta}_{rk} $$
$$ SE(\hat{\theta}_k)_{CV} =  \sqrt{ \sum_{r} w_{r}^2 SE(\hat{\theta}_{rk})^2 }$$
CV-based confidence intervals can then be formed, $\left[\hat{\theta}_{k} \pm  1.96*SE(\hat{\theta}_k)_B \right]$. 
